...
```{r}
results<-read.table("Results_multideconv/Deconvolution_after_subgrouping_deconv_subsgroups.csv", sep = ",", header=T)
...
ggplot(clinical.data, aes(x = cluster, fill = Moffitt.latest)) +
  geom_bar(position = "fill") +
  ylab("Proportion")

ggplot(clinical.data, aes(x = cluster, fill = liver.mets.at.dx)) +
  geom_bar(position = "fill") +
  ylab("Proportion")
```
...
```{r}
res_fisher = cell.groups.fisher.test(cell_groups, clinical.data, "Best.Confirmed.Overall.Response", pval = 0.01)
res_anova = cell.groups.anova.test(cell_groups, clinical.data, "Best.Confirmed.Overall.Response", pval = 0.01)
```
