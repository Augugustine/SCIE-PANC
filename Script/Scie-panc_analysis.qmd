...
pca_data <- as.data.frame(pca_viper$x)
pca_data$PatientID <- colnames(vst_matrix)
pca_data$Sex <- clinical$Gender
pca_data$Alive <- clinical$Alive.
pca_data$kras_mut <- clinical$KRAS_mut 
...
barplot(var_explained[1:50], main = "Explained Variance (%)",
        xlab = "Principal Component", ylab = "Explained Variance (%) on the TF activity",names.arg = 1:50,
        col = "skyblue")
...
```{r}
pca_df <- data.frame(pca_viper$x, Condition = your_conditions_vector)
ggplot(pca_viper, aes(x=PC1, y=PC2)) +
  geom_point(size=3) +
  theme_minimal()
...
plot<-pheatmap(viper_activity[var_genes, ], scale = "row", show_rownames = TRUE, show_colnames = FALSE, annotation_col = annot, clustering_method = "complete", main="Heatmap of Transcription Factor Activity on top 100", annotation_colors = my_colors)
plot
...
plot<-pheatmap(viper_activity, scale = "row", show_rownames = FALSE, show_colnames = FALSE, annotation_col = annot, clustering_method = "complete", main="Heatmap of Transcription Factor Activity", annotation_colors = my_colors)
plot
