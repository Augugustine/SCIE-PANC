...
library(patchwork)
# Change minMod = 30 in function compute.WTCNA in CellTFusion
...
compute.metada.association(network[[1]], clinical.data, pval = 0.05, width = 10, "Modules-Clinical") 
...
deconv_subgroups = deconvolution_dictionary(dt, pathways)

#Association of TFs modules with deconvolution 
compute.modules.relationship(network[[1]], deconv_subgroups[["Deconvolution matrix"]], "Deconvolution-TFs_Modules", vertical = T, height = 30, width = 10, pval = 0.05)
cell_groups = construct_cell_groups(counts.norm, tfs, deconv, network, deconv_subgroups, pval = 0.05)
```

```{r}
res_fisher = cell.groups.fisher.test(cell_groups, clinical.data, "Best.response.1st.line", pval = 0.01)
# Better to use the wilcox test than Fisher
res_anova = cell.groups.anova.test(cell_groups, clinical.data, "Best.response.1st.line", pval = 0.01)
res_wilcox = cell.groups.wilcox.test(cell_groups, clinical.data, "Best.Response", pval = 0.01)
tumor_response<-clinical.data
tumor_response<-tumor_response[,"Best.tumor.response"]
tumor_response<-as.data.frame(tumor_response)
res_person = compute.metada.association(cell_groups[[1]], tumor_response, width = 6, height = 15, file.name="Pearson-clinical")
# correlated with : Dendrogram_green_group_1, Dendrogram_turquoise_group_2, Dendrogram_blue_group_6, Dendrogram_blue.group_combined_5, Dendrogram_red.group_combined_7 

cell_gp<-cell_groups[[1]]
cell_gp$PCSI.ID<-rownames(cell_gp)
clinical.bis<-clinical.data
clinical.bis$PCSI.ID<-rownames(clinical.bis)
clinical.bis<-clinical.bis[,c("Best.tumor.response", "PCSI.ID")]

tumor_size <- merge(cell_gp, clinical.bis, by="PCSI.ID")
tumor_size<-tumor_size[,c("Dendrogram_green_group_1", "Dendrogram_turquoise_group_2", "Dendrogram_blue_group_6", "Dendrogram_blue.group_combined_5", "Dendrogram_red.group_combined_7","Best.tumor.response")]

ggplot(tumor_size, aes(x = Best.tumor.response, y = Dendrogram_blue_group_6)) +
  geom_point() +
  labs(title = "Relation entre taille tumorale et score du groupe cellulaire",
       x = "Taille tumorale",
       y = "Score groupe cellulaire") +
  theme_minimal()

```

Compute relationship between deconvolution and TF module for the myeloid and lymphoid lineage

```{r}
# Only immune cells 
# Lymphoid lineage
dt.lymp<-deconv_subgroups[["Deconvolution matrix"]]
...
relationship.lymp<-compute.modules.relationship(network[[1]], dt.lymp, "Deconvolution(lymph)-TFs_Modules", vertical = T, height = 15, width = 15, pval = 0.05, return=F, plot=TRUE)
#plot_heatmap(relationship.lymp, width = 12, height = 12, lineage="lymphoid")
```

```{r}
# Myeloid lineage
dt.myel<-deconv_subgroups[["Deconvolution matrix"]]
...
relationship.myel<-compute.modules.relationship(network[[1]], dt.myel, "Deconvolution(myel)-TFs_Modules", vertical = T, height = 15, width = 15, pval = 0.05, return=F, plot=T)
#plot_heatmap(relationship.myel, width = 12, height = 12)
```

Compute relationship between deconvolution and pathways for the myeloid and lymphoid lineage

```{r}
compute.modules.relationship(res[[4]], dt.myel, "Deconvolution(myel)-pathways", vertical = T, height = 15, width = 15, pval = 0.05, return=F, plot=T)
compute.modules.relationship(res[[4]], dt.lymp, "Deconvolution(lymph)-pathways", vertical = T, height = 15, width = 15, pval = 0.05, return=F, plot=T)
```

Survival curve by features

```{r}
clinical$Status<-ifelse(clinical$Alive. == "Alive", FALSE, TRUE)
compute.survival.by.features(cell_groups[[1]], clinical, "survival.days", PFS_event = "Status", time_unit = "day", max_factors = 1)
```
